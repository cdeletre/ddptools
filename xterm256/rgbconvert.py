...
class RGBconvert:
    ...
    def __init__(self,box=False):
        ...
        self.xtermlab256 = list( map(lambda rgb: RGBconvert.rgb2lab(rgb),
                        RGBconvert.XTERM256RGB))

        if box:
            self.printchar = RGBconvert.BOX
        else:
            self.printchar = RGBconvert.DOT
        # Already calculated correspondance look-up hashtable
        self.fast_rgb2xterm256 = dict()  
    ...
    def rgb2xterm256_lab(self,r,g,b):
        ...
        if rgb in self.fast_rgb2xterm256.keys():
            return self.fast_rgb2xterm256[rgb]
        ...
        lab2 = self.xtermlab256[0]
        min_distance = RGBconvert.distance_manhattan(lab1,lab2)
        best_match = i
        i = 1

        # Find the best matching xterm256color (minimal distance)
        while min_distance > 0 and i < len(self.xtermlab256):

            lab2 = self.xtermlab256[i]
            cur_distance = RGBconvert.distance_manhattan(lab1,lab2)
        ...
        self.fast_rgb2xterm256[rgb] = best_match

        return best_match
    
    def frame2ascii(self,frame,width=0,height=0):
        ...
        for y in range(height):
            ...
            for x in range(width):
                ...
                color = self.rgb2xterm256_lab(r,g,b)

                # Append pixel as a colored char (dot or square)
                ascii += ('\033[38;5;%sm%s' % (color, self.printchar))
